...
export const sectionPath = (lang: string, section: string, system = section === "site"): string => system ? `/${lang}` : `/${lang}/${encodeURIComponent(section)}`;
...
export const infoFilePath = (lang: string, section: string, slug: string, system = section === "site"): string => `${sectionPath(lang, section, system)}/${encodeURIComponent(slug)}`;
