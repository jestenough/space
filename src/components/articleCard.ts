...
export const createArticleListItem = (lang: Lang, article: ArticleMeta): HTMLLIElement => {
  const item = createElement("li", "article-card");
...
export const createEmptyArticleItem = (lang: Lang): HTMLLIElement => createElement("li", "meta", text(lang).noArticles);
...
const makeCardLink = (
  ...
  const link = createElement("a", "article-card-link article-card-full");
  link.href = href;
  link.dataset.internal = "true";
  link.dataset[datasetKey] = datasetValue;
  link.append(createElement("strong", undefined, title));

  for (const line of metaLines) {
    if (typeof line === "string") {
        link.append(createElement("div", "meta", line));
      continue;
    }

    const row = createElement("div", "meta tag-line");
    for (const chunk of line) row.append(createElement("span", chunk.className, chunk.text), document.createTextNode(" "));
    link.append(row);
  }
