...
import { DEFAULT_HREFLANG, OG_LOCALE_BY_LANG } from "./config";

type SeoArgs = { lang: Lang; title: string; description: string; indexable?: boolean; canonicalPath?: string; alternatePaths?: Partial<Record<Lang | typeof DEFAULT_HREFLANG, string>>; type?: "website" | "article"; structuredData?: unknown[] };

export function updateSeoMeta(args: SeoArgs): void {
  ...
  ensureMeta('meta[property="og:locale"]', { property: "og:locale" }).content = OG_LOCALE_BY_LANG[args.lang];
  ...
  for (const [hreflang, path] of Object.entries(args.alternatePaths ?? {})) {
    ...
    link.rel = "alternate"; link.hreflang = hreflang; link.href = window.location.origin + path; link.dataset.managedSeo = "true";
  ...
  if (args.structuredData?.length) {
    ...
    script.id = "structured-data"; script.type = "application/ld+json"; script.textContent = JSON.stringify(args.structuredData.length === 1 ? args.structuredData[0] : args.structuredData);
...
const ensureMeta = (selector: string, attrs: Record<string, string>): HTMLMetaElement => {
  ...
  if (!meta) { meta = document.createElement("meta"); for (const [key, value] of Object.entries(attrs)) meta.setAttribute(key, value); document.head.append(meta); }
  return meta;
};
